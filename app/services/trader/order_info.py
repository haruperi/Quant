"""MQL5-compatible OrderInfo class wrapping pending order properties.

Provides access to the active pending order properties.
"""
...
class OrderInfo:
    """Provides access to properties of a pending order."""

    def __init__(self, ticket: int | None = None) -> None:
        """Initialize the OrderInfo helper.
    ...
    def select(self, ticket: int) -> bool:
        """Select a pending order by ticket.
        ...
        orders = broker.get_order_info(ticket=ticket)
        if orders:
            self._data = orders[0]
            return True
        self._data = None
    ...
    def ticket(self) -> int:
        """Get the ticket of the pending order.
    ...
    def time_setup(self) -> int:
        """Get the setup time of the pending order.
    ...
    def time_expiration(self) -> int:
        """Get the expiration time of the pending order.
    ...
    def time_done(self) -> int:
        """Get the execution/cancellation time of the pending order.
    ...
    def type(self) -> int:
        """Get the type of the pending order.
    ...
    def type_time(self) -> int:
        """Get the expiration type of the pending order.
    ...
    def type_filling(self) -> int:
        """Get execution type of the pending order.
    ...
    def state(self) -> int:
        """Get status of the pending order.
    ...
    def magic(self) -> int:
        """Get the magic number of the pending order.
    ...
    def volume_initial(self) -> float:
        """Get the initial volume of the pending order.
    ...
    def volume_current(self) -> float:
        """Get the current volume of the pending order.
    ...
    def price_open(self) -> float:
        """Get the price level of the pending order.
    ...
    def symbol(self) -> str:
        """Get the symbol of the pending order.
    ...
    def comment(self) -> str:
        """Get the comment of the pending order.
