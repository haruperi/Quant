# ruff: noqa: PLR2004
"""Usage example for CTraderClient broker client and wrappers."""
...
from app.services.brokers.ctrader import (  # noqa: E402
    get_account_info,
    get_ctrader_client,
...
def example_01_connect() -> None:
    """Demonstrate cTrader Client connection and usage wrapper functions."""
    print("\n" + "=" * 100)
    print("--- 1. Connecting and Retrieving cTrader Data via Wrappers ---")
    print("=" * 100)
    ...
    if client.is_connected():
        print("cTrader connection established successfully.")
    else:
        print("Failed to establish cTrader connection.")
...
def example_03_account() -> None:
    """Demonstrate cTrader Account class."""
    print("\n" + "=" * 100)
...
def example_04_symbol() -> None:
    ...
    symbol = get_symbol_info(trading_symbol)
...
def example_05_position() -> None:
    ...
    print("\n" + "=" * 60)
    print(f"Selecting by Symbol '{trading_symbol}'")
    print("=" * 60)
    eur = get_position_info(symbol=trading_symbol)
    if eur:
        position = eur[0]
        print(f"Found {trading_symbol} position:")
        print(f"  Ticket: {position.ticket}")
        print(f"  Profit: {position.profit}")
    else:
        print(f"No {trading_symbol} position found.")
...
def example_06_order() -> None:
    ...
    print(f"Selecting by Symbol '{trading_symbol}'")
    print("=" * 60)
    eur = get_order_info(symbol=trading_symbol)
    if eur:
        order = eur[0]
        print(f"Found {trading_symbol} order:")
        print(f"  Ticket: {order.ticket}")
        print(f"  State: {order.state}")
    else:
        print(f"No {trading_symbol} order found.")
...
def example_07_history_order() -> None:
    ...
    filled_count = sum(
        1
        for o in orders
        if int(o.state) == 4  # FILLED
    )
    canceled_count = sum(
        1
        for o in orders
        if int(o.state) == 2  # CANCELED
    )
...
def example_09_open_position() -> None:
    ...
    print("\n" + "=" * 100)
    print(f"--- 9. Opening a Position (Buy 0.02 {trading_symbol}) ---")
    print("=" * 100)
...
def example_10_calc_profit_margin() -> None:
    """Demonstrate profit and margin calculations."""
    print("\n" + "=" * 100)
...
def example_17_shutdown() -> None:
    """Demonstrate shutting down the cTrader connection."""
    print("\n" + "=" * 100)
    print("--- 17. Shutting down cTrader Connection ---")
    print("=" * 100)

    client.disconnect()
    print("cTrader connection shut down.")
